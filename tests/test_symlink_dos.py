...
def test_self_referential_symlink():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"✓ Self-referential symlink with cycle detection (follow_symlinks=True): {len(results2)} files in {elapsed2:.4f}s"
        )
...
def test_cross_directory_symlink_cycle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"✓ Cross-directory cycle with detection (follow_symlinks=True): {len(results2)} files in {elapsed2:.4f}s"
        )
