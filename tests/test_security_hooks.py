...
def test_hooks_env_var_logic():
    ...
    for env_value, expected_enabled in test_cases:
        ...
        assert (
            hooks_enabled == expected_enabled
...
def test_code_contains_security_checks():
    ...
    assert "GHERKIN_PARSER_ENABLE_HOOKS" in content, "Library.py should check GHERKIN_PARSER_ENABLE_HOOKS env var"
    assert "self._hooks_enabled" in content, "Library.py should have _hooks_enabled attribute"
    assert 'os.getenv("GHERKIN_PARSER_ENABLE_HOOKS"' in content, "Library.py should call os.getenv for hooks"

    # Count occurrences of the security check
    check_count = content.count("if not self._hooks_enabled:")
    assert check_count >= 3, f"Library.py should have at least 3 hooks_enabled checks (found {check_count})"

    # Check for teardown fix
    assert (
        "data.teardown.config(" in content
    ...
    assert (
        "GHERKIN_PARSER_AUTO_IMPORT_RESOURCES" in content
    ), "gherkin_builder.py should check GHERKIN_PARSER_AUTO_IMPORT_RESOURCES env var"
    assert "auto_import" in content, "gherkin_builder.py should have auto_import variable"
    assert (
        'os.getenv("GHERKIN_PARSER_AUTO_IMPORT_RESOURCES"' in content
    ), "gherkin_builder.py should call os.getenv for auto-import"
    assert "if auto_import:" in content, "gherkin_builder.py should conditionally import resources"
...
def test_documentation_strings():
    ...
    content = library_py.read_text()
    assert (
        "Security hardening" in content or "security" in content.lower()
