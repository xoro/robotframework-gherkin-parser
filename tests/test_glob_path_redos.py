...
def test_no_nested_quantifiers():
    ...
    for pattern in test_cases:
        ...
        assert "((?:[^/]*(?:/|$))*)" not in regex, \
            f"Pattern {pattern} still contains nested quantifier!"
        
        # The fixed pattern should use (?:[^/]+/)* or .* instead
        assert "(?:[^/]+/)*" in regex or ".*" in regex, \
            f"Pattern {pattern} doesn't use the fixed pattern!"
...
def test_performance_worst_case():
    ...
    for pattern, path_str in test_cases:
        ...
        assert status != "timeout", \
            f"Pattern {pattern} timed out - ReDoS vulnerability may still exist!"
        
        # Should complete very quickly (well under timeout)
        assert elapsed < 0.5, \
            f"Pattern {pattern} took {elapsed:.4f}s - too slow, potential performance issue!"
...
def test_basic_functionality():
    ...
    for pattern, path, should_match in test_cases:
        ...
        print(f"Pattern: {pattern:30} Path: {path:35} Expected: {should_match:5} Got: {result:5} {'✓' if result == should_match else '✗'}")
